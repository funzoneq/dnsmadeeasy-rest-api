...
RSpec.describe DnsMadeEasy::Rest::Api::Client do
  let(:api_domain) { DnsMadeEasy::PRODUCTION_API_ENDPOINT }
  ...
  describe '#get_id_by_domain' do
    ...
    it 'returns the id of the domain' do
      stub_request(:get, api_domain + '/dns/managed/id/something.wanelo.com')
  ...
  describe '#domain' do
    ...
    before do
      expect(subject).to receive(:get_id_by_domain).with('something.wanelo.com').and_return(123)
    end
  ...
  describe '#find' do
    ...
    before do
      expect(subject).to receive(:records_for).with('something.wanelo.com').and_return(records_for_response)
    end
  ...
  %w[a aaaa ptr txt cname ns spf].each do |record_type|
    ...
    describe "##{method_name}" do
      ...
      it "calls through to create record with \"#{upper_record_type}\"" do
        expect(subject).to receive(:create_record).with('something.wanelo.com',
                                                        'smellyface',
                                                        upper_record_type,
                                                        '192.168.1.1', {}).and_return({})
        expect(subject.send(method_name, 'something.wanelo.com',
  ...
  describe '#create_srv_record' do
    ...
    it 'creates an srv record' do
      ...
      expect(subject.create_srv_record('something.wanelo.com', 'serv',
                                       priority,
                                       weight,
                                       port,
                                       '192.168.1.1')).to eq({})
  ...
  describe '#create_httpred_record' do
    ...
    it 'creates an srv record' do
      ...
      expect(subject.create_httpred_record('something.wanelo.com',
                                           'redirect',
                                           '192.168.1.1',
                                           redirect_type,
                                           description,
